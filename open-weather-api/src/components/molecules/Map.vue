...
// DOM 요소가 완전히 로드된 후에 initMap 함수를 호출함. nextTick
...
function initMap() {
  ...
  positions.forEach((item) => {
    ...
    kakao.maps.event.addListener(marker, "click", () => {
      ...
      console.log(res);
      //  store을 호출했을 때 payload로 넘겨주는 값.
      // res에서 새로운 데이터를 추출해서 (axis)라는 객체 데이터를 만들어주고
      //
      const axis = {
        lat: res.Ma,
        lon: res.La,
      ...
      // store의 openWeatherApi를 호출해서 payload로 넘겨줌.
